...
const useActiveSection = (sectionIds) => {
  ...
  const getCurrentSection = useCallback(() => {
    ...
    console.log(
      `Fallback - closest section: ${closestSection} (index: ${closestIndex})`
    );
  ...
  const updateActiveSection = useCallback(
    (newSection, newIndex, source = "unknown") => {
      if (newSection !== activeSection || newIndex !== activeSectionIndex) {
        console.log(
          `[${source}] Updating section from "${activeSection}" (${activeSectionIndex}) to "${newSection}" (${newIndex})`
        );
