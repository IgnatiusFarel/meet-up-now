...
const Meeting = () => {
  ...
  const [pinnedParticipantId, setPinnedParticipantId] = useState(1); // Default pin participant dengan id 1
  const [viewMode, setViewMode] = useState('gallery'); // 'gallery' atau 'focus'
  ...
  const participants = [
    { id: 1, name: "John Doe", isPresenter: true, micOn: true, camOn: true, isMe: true },
    { id: 2, name: "Sarah Wilson", isPresenter: false, micOn: true, camOn: true, isMe: false },
    { id: 3, name: "Mike Chen", isPresenter: false, micOn: false, camOn: true, isMe: false },
    { id: 4, name: "Emily Davis", isPresenter: false, micOn: true, camOn: false, isMe: false },
  ...
  // Get pinned participant (yang ditampilkan di layar utama)
  const pinnedParticipant = participants.find(p => p.id === pinnedParticipantId) || participants[0];
  // Get other participants (yang ditampilkan di sidebar)
  ...
  const ParticipantCard = ({ participant, isMain = false, onPin, isGalleryMode = false }) => (
    <div 
      className={`relative bg-gray-900 rounded-xl overflow-hidden ${
        isMain && !isGalleryMode ? 'h-full' : isGalleryMode ? 'h-full' : 'h-32'
    ...
    >
      {participant.camOn ? (
        <div className="w-full h-full bg-gradient-to-br from-blue-600 via-purple-600 to-pink-600 flex items-center justify-center">
          <div className="text-white text-center">
            <div className={`${isMain && !isGalleryMode ? 'text-8xl' : isGalleryMode ? 'text-6xl' : 'text-4xl'} mb-2`}>👤</div>
            {isMain && !isGalleryMode && <p className="text-xl opacity-80">Camera Active</p>}
          </div>
        </div>
      ) : (
        <div className="w-full h-full bg-gray-800 flex items-center justify-center">
          <div className="text-center text-gray-400">
            <div className={`${isMain && !isGalleryMode ? 'text-8xl' : isGalleryMode ? 'text-6xl' : 'text-4xl'} mb-2`}>📷</div>
            <p className={`${isMain && !isGalleryMode ? 'text-xl' : isGalleryMode ? 'text-base' : 'text-sm'}`}>Camera Off</p>
          </div>
        </div>
      )}

      {/* Pin/Unpin Button - hanya muncul saat hover dan bukan main view atau di gallery mode */}
      ...
      {/* Pinned Indicator - untuk yang sedang di-pin di main view (hanya di focus mode) */}
      {isMain && !isGalleryMode && (
      ...
      {/* Name Overlay */}
      <div className="absolute bottom-2 left-2 bg-black bg-opacity-70 text-white px-3 py-1 rounded-lg backdrop-blur-sm">
        <div className="flex items-center space-x-2">
          <Avatar size={isMain && !isGalleryMode ? 28 : 20} className="bg-blue-600">
            {participant.name.charAt(0)}
          </Avatar>
          <span className={`${isMain && !isGalleryMode ? 'text-sm' : 'text-xs'} font-medium`}>
      ...
      {/* Speaking Indicator */}
      {participant.micOn && (
        <div className={`absolute ${isMain && !isGalleryMode ? 'top-4 left-4' : 'top-2 left-2'} ${participant.isPresenter && (!isMain || isGalleryMode) ? 'top-8 left-2' : ''}`}>
          <div className="w-3 h-3 bg-green-400 rounded-full animate-pulse"></div>
        </div>
      )}

      {/* Click to Pin Hint - hanya di focus mode */}
      {!isMain && !isGalleryMode && (
            ...
            {viewMode === 'focus' ? (
              <>
                ...
                <div className="flex-1 pr-4">
                  <ParticipantCard participant={pinnedParticipant} isMain={true} />
                </div>
              ...
              /* Gallery Mode: All participants same size */
              <div className="w-full grid grid-cols-4 gap-4 auto-rows-fr overflow-y-auto">
            ...
            <div className="flex border-b">
              <button
                ...
                className={`flex-1 px-4 py-3 text-sm font-medium ${
                  showParticipants ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-600'
                }`}
              >
              ...
              <button
                ...
                className={`flex-1 px-4 py-3 text-sm font-medium ${
                  showChat ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-600'
                }`}
              >
                💬 Chat
              </button>
                  ...
                  {participants.map((participant) => (
                    <div key={participant.id} className="flex items-center justify-between p-2 hover:bg-gray-50 rounded-lg">
                      ...
                      <div className="flex space-x-1">
                        <span className={participant.micOn ? 'text-green-500' : 'text-gray-400'}>
                          {participant.micOn ? '🎤' : '🎤🔇'}
                        </span>
                        <span className={participant.camOn ? 'text-green-500' : 'text-gray-400'}>
                          {participant.camOn ? '📹' : '📹🔇'}
                        </span>
                      </div>
                    </div>
      ...
      <div className="bg-white border-t px-6 py-4">
        <div className="flex items-center justify-between">
          {/* Left Controls */}
            ...
            <span className="text-sm text-gray-600">{currentTime} <span>|</span> 123-456-789</span>        
          ...
          <div className="flex items-center space-x-4">
            ...
            <Button
              ...
              className={`w-12 h-12 rounded-full flfex items-center justify-center transition-all ${
                micOn
            ...
            <Button
              ...
              type="text"
              title={viewMode === 'gallery' ? 'Switch to Focus Mode' : 'Switch to Gallery Mode'}
            >
