    ...
    this.io.on("connection", (socket) => {
      ...
      socket.on("join-meeting", async (data) => {
        try {
          ...
          const participants = await this.getMeetingParticipants(
            meeting.meetingId
          );
      ...
      socket.on("update-participant-status", async (data) => {
        try {
          ...
          if (typeof isScreenShare === "boolean")
            updates.isScreenShare = isScreenShare;
      ...
      // Socket for Artboard Feature
      socket.on("artboard:draw", async (data) => {
        try {
          const { meetingId, pathData } = data;
          ...
          // PERBAIKAN: Gunakan socket.to() bukan this.io.to()
          // Ini akan broadcast ke semua user KECUALI sender
          socket.to(roomName).emit("artboard:draw", {
            pathData,
            userId: socket.userId,
            user: socket.user,
            timestamp: new Date(),
          ...
          console.log(
            `Artboard draw event from ${socket.user.name} in meeting ${meetingId}`
          );
        } catch (error) {
          socket.emit("error", { message: error.message });
        }
      ...
      socket.on("artboard:erase", async (data) => {
        try {
          const { meetingId, eraseData } = data;
          ...
          // PERBAIKAN: Gunakan socket.to() bukan this.io.to()
          socket.to(roomName).emit("artboard:erase", {
            eraseData,
            userId: socket.userId,
            user: socket.user,
            timestamp: new Date(),
          ...
          console.log(
            `Artboard erase event from ${socket.user.name} in meeting ${meetingId}`
          );
        } catch (error) {
          socket.emit("error", { message: error.message });
        }
      ...
      socket.on("artboard:clear", async (data) => {
        try {
          const { meetingId } = data;
          ...
          // PERBAIKAN: Gunakan socket.to() bukan this.io.to()
          socket.to(roomName).emit("artboard:clear", {
            userId: socket.userId,
            user: socket.user,
            timestamp: new Date(),
          });
          ...
          console.log(
            `Artboard clear event from ${socket.user.name} in meeting ${meetingId}`
          );
        } catch (error) {
          socket.emit("error", { message: error.message });
        }
